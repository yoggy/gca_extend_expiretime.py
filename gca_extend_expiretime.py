...
for gca in gca_list["anchors"]:
  ...
  rv = subprocess.check_output(["curl","-H",f"Authorization: Bearer {bearer_token}","-H","Content-Type: application/json","-X","PATCH",f"https://arcore.googleapis.com/v1beta2/management/anchors/{anchor_id}?updateMask=expire_time","-d","{expireTime:\"9999-12-31T00:00:00Z\"}"],stderr=subprocess.DEVNULL)
